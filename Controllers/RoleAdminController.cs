    ...
    public class RoleAdminController : Controller
    {
        ...
        public async Task<ActionResult> Create(RegisterViewModel model, string roleName)
        {
            if (ModelState.IsValid)
            {
                ...
                // Attempt to create the new user using the user manager
                IdentityResult result = await _userManager.CreateAsync(user, model.Password);

                // If the user was created successfully, assign the specified role
                if (result.Succeeded)
                {
                    // Assign the user to the specified role (e.g., "Employee")
                    await _userManager.AddToRoleAsync(user, roleName);

                    return RedirectToAction("Index");
                }
                else // User was not added successfully, so add errors to model 
